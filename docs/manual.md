...
Everything the app does, at length: the panes, the settings, the rules that are
not obvious from looking at them, and how two instances of the bridge coexist.

[`README.md`](../README.md) is the short version — what this is, how to install
it, and how the pieces fit together. Start there if you have not already.
...
bridge: a terminal, VS Code, a background agent, another TGXCode window.
...
in `plans/02-notifications-and-shell.md`. And a desktop's own do-not-disturb
...
**TGXCode bridge** — killing one takes its turns with it, since `claude`
...
`the Android client repository` — a separate repository, and a client of this bridge exactly
as the desktop is.

**It is a client, not a second version of this app.** Everything it shows comes over
the same API, written down as a contract in [`docs/api.md`](api.md); it adds no
...
Reaching the bridge from outside your network is a deployment choice, and
[`remote.md`](remote.md) is the runbook. The short version is
...
where it is. That matters on any network you do not control.
...
reasoning is in `plans/14-bridge-security.md` §C and the table in
`remote.md`.
...
```bash
alias restart-bridge='bash ~/src/tgxcode/scripts/restart-bridge.sh'
```
...
SHELL=/bin/bash
MAILTO=""
0 0 * * * /path/to/tgxcode/scripts/restart-bridge.sh >/tmp/bridge.log 2>&1
```
