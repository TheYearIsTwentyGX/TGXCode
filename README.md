...
| `web/app.js` | Everything not yet moved out of it: the event stream, the rail's data, the panel machinery the boards open and close through, snippets, and the boot and event wiring that calls every module's `wire*()` at the point its listeners always registered. Imports the modules below |
