...
| 9 | [Composer](docs/plans/09-composer.md) | `@` files, `/` commands, images, a visible send queue, retry, snippets. The `/` half of A shipped, and `@` now exists for **sessions** — the picker and its group headings are built, so the file half is a second source rather than a second menu. B shipped as **files**, not just images: paste or drop anything, it lands in `attached_assets/` at the root of the session's checkout, and an image goes inline as well so it needs no `Read`. E shipped, and took the LGTM button with it — that button is now the seeded first snippet rather than hard-coded text, and any snippet can be pinned to a button of its own. Its placeholders are declared per snippet rather than resolved from the session, which is a different feature from the one this plan sketched; see the section for why. D is what is left. |
